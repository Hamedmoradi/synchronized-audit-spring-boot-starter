package ir.bmi.audit.reactive.filter;

import ir.bmi.audit.reactive.interceptor.RequestLoggingInterceptor;
...
import ir.bmi.audit.reactive.interceptor.ResponseLoggingInterceptor;
...
public class ReactiveSpringLoggingFilter implements WebFilter {
	...
	private void logResponse(long startTime, ServerHttpResponse response, int overriddenStatus) {
		...
		if (useContentLength && (header == null || header.get(0).equals("0"))) {
			if (logHeaders)
				LOGGER.info("Response({} ms): status={}, headers={}, audit={}");
			else
				LOGGER.info("Response({} ms): status={}, audit={}");
		}
	}
